...
# check DEGs of transition FB (cluster 9)

A <- names(sc1000_LAD_Cryo_Sham_Fibro@cpart)[sc1000_LAD_Cryo_Sham_Fibro@cpart %in% c(1:8, 10:19)]
B <- names(sc1000_LAD_Cryo_Sham_Fibro@cpart)[sc1000_LAD_Cryo_Sham_Fibro@cpart %in% c(9)]
x <- diffexpnb(getfdata(sc1000_LAD_Cryo_Sham_Fibro,n=c(A,B)), A=A, B=B )
plotdiffgenesnb(x,pthr=.05,lthr=.5,mthr=-1,Aname="FB/myoFB",Bname="tFB",show_names=TRUE,padj=TRUE)
...
dg <- clustdiffgenes(sc1000_LAD_Cryo_Sham_Fibro,c(9),pvalue=0.03)
head(dg$dg)
